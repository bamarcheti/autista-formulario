...
export function BeneficiarySelector({ value, onChange }: BeneficiarySelectorProps) {
  return (
    <div className="space-y-3">
      <label className="block text-sm font-medium text-foreground/90">
        Para quem é o benefício BPC/LOAS? <span className="text-destructive">*</span>
      </label>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        >
          <div className={cn(
            ...
            value === "proprio" ? "bg-primary text-primary-foreground" : "bg-muted"
          )}>
        ...
        >
          <div className={cn(
            "w-10 h-10 rounded-full flex items-center justify-center shrink-0",
            value === "outro" ? "bg-primary text-primary-foreground" : "bg-muted"
          )}>
            <Users className="w-5 h-5" />
          </div>
          <div className="text-left">
            <p className="font-medium text-sm">Para outra pessoa</p>
            <p className="text-xs text-muted-foreground">Filho, dependente, etc.</p>
          </div>
        </button>
      </div>
    </div>
