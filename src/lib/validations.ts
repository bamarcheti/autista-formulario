...
export function formatPhone(value: string): string {
  ...
  if (d.length >= 7) return `(${d.slice(0, 2)}) ${d.slice(2, 7)}-${d.slice(7, 11)}`;
  return d;
}
...
function formatRgBase(d: string): string {
  const n = d.length;
  if (n >= 8) {
    const a = d.slice(0, 2), b = d.slice(2, 5), c = d.slice(5, 8);
    let out = `${a}.${b}.${c}`;
    let r = d.slice(8);
    while (r.length) {
...
export function formatRG(value: string): string {
  let raw = (value || "").toUpperCase().replace(/[^0-9A-Z.\-\s]/g, '').replace(/\s+/g, ' ').trimStart();
...
export function capitalizeName(str: string): string {
  ...
  return str.toLowerCase().trim().split(/\s+/).map(w => 
