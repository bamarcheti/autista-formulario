...
export function normalizeProfissao(raw) {
  // 1) Mantém só letras/números/espaços e colapsa espaços
  let v = raw
    .replace(/[^\p{L}\p{N} ]+/gu, "") // remove qualquer coisa fora da regra
    .replace(/\s+/g, " ") // múltiplos espaços -> 1
    .trimStart(); // evita começar com espaço

  // 2) Coloca tudo em minúsculo e capitaliza cada palavra
  v = v
    .toLocaleLowerCase("pt-BR")
    .split(" ")
    .map((w) => (w ? w[0].toLocaleUpperCase("pt-BR") + w.slice(1) : ""))
    .join(" ");
